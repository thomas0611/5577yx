...
namespace Game.Manager
{
    public class Game_Xyb : IGame
    {
        ...
        public string Login(int UserId, int ServerId, int IsPC)
        {
            ...
            string url = string.Format("op_id={0}&sid={1}&game_id={2}&account={3}&time={4}",
                    gc.AgentId, gs.ServerNo, 20, gu.UserName, tstamp);     //获取验证字符串
        ...
        public string Pay(string OrderNo)
        {
            ...
            if (gus.IsGameUser(gu.UserName))                                //判断用户是否属于平台
            {
                ...
                string url = string.Format("op_id={0}&sid={1}&game_id={2}&account={3}&order_id={4}&game_money={5}&u_money={6}&time={7}",
                                          gc.AgentId, gs.ServerNo, 20, gu.UserName, OrderNo, PayGold, order.PayMoney, tstamp);        //获取验证字符串
                Base64Protector bp = new Base64Protector();
                Auth = bp.Base64Code(url);                                      //获取验证码
        ...
        public GameUserInfo Sel(int UserId, int ServerId)
        {
            ...
            try
            {
                ...
                gui = new GameUserInfo(gu.Id.ToString(), gu.UserName, Utils.ConvertUnicodeStringToChinese(b[0].Substring(12).Replace("\"", "")), int.Parse(b[1].Substring(8).Replace("\"", "")), gs.Name, os.GetOrderInfo(gu.UserName), "Success");
            }
            catch (Exception)
            {
                gui.UserName = "没有角色";
