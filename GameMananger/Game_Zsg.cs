...
namespace Game.Manager
{
    public class Game_Zsg : IGame
    {
        ...
        public string Login(int UserId, int ServerId, int IsPc)
        {
            ...
            Sign = DESEncrypt.Md5("from=" + gc.AgentId + "game=zsg" + "server=" + gs.ServerNo + "user_id=" + gu.UserName + "fatigue=2" + "t=" + tstamp + "login_secret_signature=" + gc.LoginTicket, 32);            //获取验证码
            string LoginUrl = "http://" + gc.LoginCom + "?from=" + gc.AgentId + "&game=zsg&server=" + gs.ServerNo + "&user_id=" + gu.UserName + "&fatigue=2&t=" + tstamp + "&signature=" + Sign;
            return LoginUrl;
        }
        ...
        public string Pay(string OrderNo)
        {
            ...
            {
                tstamp = Utils.GetTimeSpan();                               //获取时间戳
                Sign = DESEncrypt.Md5("from=" + gc.AgentId + "game=zsg" + "server=" + gs.ServerNo + "user_id=" + gu.UserName + "amount=" + order.PayMoney * 100 + "order_number=" + OrderNo + "t=" + tstamp + "transfer_secret_signature=" + gc.PayTicket, 32);
                string PayUrl = "http://" + gc.PayCom + "?from=" + gc.AgentId + "&game=zsg&server=" + gs.ServerNo + "&user_id=" + gu.UserName + "&amount=" + order.PayMoney * 100 + "&order_number=" + OrderNo + "&t=" + tstamp + "&signature=" + Sign;
        ...
        public GameUserInfo Sel(int UserId, int ServerId)
        {
            ...
            Sign = DESEncrypt.Md5("from=" + gc.AgentId + "game=zsg" + "server=" + gs.ServerNo + "user_id=" + gu.UserName + "t=" + tstamp + "transfer_secret_signature=" + gc.SelectTicket, 32);         //获取验证码
            string SelUrl = "http://" + gc.ExistCom + "?from=" + gc.AgentId + "&game=zsg&server=" + gs.ServerNo + "&user_id=" + gu.UserName + "&t=" + tstamp + "&signature=" + Sign;
            ...
            try
            {
                ...
                gui = new GameUserInfo(b[4].Substring(9), gu.UserName, b[5].Substring(11).Replace("\"", ""), int.Parse(b[3].Substring(8)), gs.Name, os.GetOrderInfo(gu.UserName), "Success");
            }
            catch (Exception)
            {
